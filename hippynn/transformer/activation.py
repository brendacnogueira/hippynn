...
class MultiheadAttention(Module):
    ...
    def __init__(
        ...
        if use_euclidean_rope:
            if rope_num_frequencies <= 0:
                raise ValueError("rope_num_frequencies must be greater than 0")
            if rope_min_frequency <= 0 or rope_max_frequency <= 0:
                raise ValueError("RoPE frequencies must be positive")
            if rope_max_frequency < rope_min_frequency:
                raise ValueError("rope_max_frequency must be >= rope_min_frequency")
            frequencies = torch.logspace(
                math.log10(float(rope_min_frequency)),
                math.log10(float(rope_max_frequency)),
                int(rope_num_frequencies),
                **factory_kwargs,
            )
    ...
    def forward(
        ...
        euclidean_rope_distances: Tensor | None = None,
        euclidean_rope_mask: Tensor | None = None,
