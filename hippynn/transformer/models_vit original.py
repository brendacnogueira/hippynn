...
# ``activation.py`` intentionally imports ``transformer.functional``.  Register
# the local package under that compatibility name without editing that file.
...
class Encoder1DBlock(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if inputs.ndim != 3:
            raise ValueError(f"Expected inputs with shape (batch, seq, hidden), got {tuple(inputs.shape)}")
